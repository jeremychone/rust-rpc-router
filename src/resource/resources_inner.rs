...
// Note: Cherry picked functions from `extensions.rs` that is used in this code base.
//       The rest are prefixed with `_` for now.
impl ResourcesInner {
...
// Note: Usued code for now (from `extensions.rs`, keep until refactor)
#[allow(unused)] // For early development.
impl ResourcesInner {
	/// Create an empty `Extensions`.
	#[inline]
	fn _new() -> ResourcesInner {
		ResourcesInner { map: None }
	}
	...
	/// assert_eq!(*ext.get::<i32>().unwrap(), 3);
	/// ```
	fn get_or_insert_with<T: Clone + Send + Sync + 'static, F: FnOnce() -> T>(&mut self, f: F) -> &mut T {
		let out = self
			.map
			.get_or_insert_with(Box::default)
			.entry(TypeId::of::<T>())
	...
	/// ```
	fn get_or_insert_default<T: Default + Clone + Send + Sync + 'static>(&mut self) -> &mut T {
...
impl fmt::Debug for ResourcesInner {
...
fn test_extensions() {
	...
	let mut extensions = ResourcesInner::_new();
