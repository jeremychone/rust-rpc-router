...
//! - Decisions on what to retain or discard will be made later.
...
impl Store {
	...
	/// ```
	pub fn _get_mut<T: Send + Sync + 'static>(&mut self) -> Option<&mut T> {
		self.map
			.as_mut()
	...
	/// ```
	pub fn _get_or_insert<T: Clone + Send + Sync + 'static>(&mut self, value: T) -> &mut T {
		self._get_or_insert_with(|| value)
	}
	...
	/// assert_eq!(*ext.get::<i32>().unwrap(), 3);
	/// ```
	pub fn _get_or_insert_with<T: Clone + Send + Sync + 'static, F: FnOnce() -> T>(&mut self, f: F) -> &mut T {
		let out = self
			.map
			.get_or_insert_with(Box::default)
			.entry(TypeId::of::<T>())
	...
	/// ```
	pub fn _get_or_insert_default<T: Default + Clone + Send + Sync + 'static>(&mut self) -> &mut T {
		self._get_or_insert_with(T::default)
	}
	...
	/// assert!(ext.get::<i32>().is_none());
	/// ```
	pub fn _remove<T: Send + Sync + 'static>(&mut self) -> Option<T> {
		self.map
			.as_mut()
	...
	/// ```
	pub fn _extend(&mut self, other: Self) {
...
fn test_extensions() {
	...
	assert_eq!(extensions._get_mut(), Some(&mut 5i32));

	let ext2 = extensions.clone();

	assert_eq!(extensions._remove::<i32>(), Some(5i32));
